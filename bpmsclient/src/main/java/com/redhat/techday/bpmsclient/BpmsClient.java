...
/**
 * 
 * This class is responsible for making calls to BPMS via its REST API. Refer to
 * https://access.redhat.com/documentation/en-US/Red_Hat_JBoss_BPM_Suite/6.1/html/Development_Guide/sect-Remote_Java_API.html#The_REST_Remote_Java_RuntimeEngine_Factory
...
public class BpmsClient {
  ...
  private void constructKieSession(String baseUrl, String deploymentId, String username, String password) {
    ...
    engine = RemoteRuntimeEngineFactory.newRestBuilder().addUrl(constructUrl(baseUrl)).addUserName(username)
  ...
  public ProcessInstance startProcess(String processId, Map<String, Object> processVariables) {
    ...
    if (LOGGER.isDebugEnabled()) {
      for (Entry<String, Object> processVar : processVariables.entrySet()) {
        LOGGER.debug("Process Variable: " + processVar.getKey() + " ===> " + processVar.getValue());
      }
    }
